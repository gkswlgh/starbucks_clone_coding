...
public class RetrofitHelper {
	...
	/**
	 * Log4j 객체를 통해 로그를 기록하도록 하는 메서드를 구현하는 클래스정의 Retrofit이 이 클래스의 객체를 통해 통신과정을 의미하는
	 * 문자열을 log() 메서드로 전달한다.
	 *
	 */
	 ...
	 * 클래스의 객체를 통신 객체에 연결해야한다. Mozilla/5.0 (Windows NT 10.0; Win64; x64)
	...
	public class MyInterceptor implements Interceptor {
		...
		public Response intercept(Chain chain) throws IOException {
			...
			Request.Builder builder = originalRequest.newBuilder(); // 요청정보객체를 복제해서 새로운객체생성
			builder.removeHeader("User-Agent"); // 기본으로 포함되어있는 브라우저 정보삭제
	 ...
	 * 
	 * @param baseUrl - 접근하고자하는 API의 기본 주소 (ex: http://도메인)
	 * @return Retrofit 객체
	 */
	// -> import retrofit2.Retrofit;
