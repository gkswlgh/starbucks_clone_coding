package hanjiho.project.starbucks.interceptor;
...
public class AppInterceptor implements HandlerInterceptor {
	...
	/**
	 * [자주 사용] Controller 실행 요청 전에 수행되는 메서드 클라이언트의 요청을 컨트롤러에게 전달하기 전에 호출된다.
	 * return값으로 boolean 값을 전달하는데 false인 경우 controller를 실행시키지 않고 요청을 종료한다. 보통 이 곳에서
	 * 각종 체크작업과 로그를 기록하는 작업을 진행한다.
	 */
	// 안전하지 않은 형변환에 대한 경고 끄는 어노테이션
...
//		log.debug("AppInterceptor.preHandle 실행됨");
		...
		/* Helper의 초기화는 모든 컨트롤러마다 개별적으로 호출되어야한다. 
	...
	/**
	 * [자주 사용] view 단으로 forward 되기 전에 수행. 컨트롤러 로직이 실행된 이후 호출된다. 컨트롤러 단에서 에러 발생 시 해당
	 * 메서드는 수행되지 않는다. request로 넘어온 데이터 가공 시 많이 사용된다.
	 */
	@Override
...
//		log.debug("AppInterceptor.postHandle 실행됨");
	...
	/**
	 * [잘 안씀] 컨트롤러 종료 후 view가 정상적으로 랜더링 된 후 제일 마지막에 실행이 되는 메서드
	 */
	@Override
